...
- On launch, `AppState.bootstrapAccounts()` calls `migrateLegacyAccountIfNeeded()` which probes GitHub `GET /user` with whichever credential currently lives under the legacy fixed keys, derives a stable `Account` (`<host>#<username>`), and copies the existing OAuth tokens / client credentials / PAT under the new account-scoped keys. The legacy GitHub entries are left in place so that downgrading to a previous build still finds them.
- After successful login, RepoBar probes the provider's `GET /user`, derives an `Account`, persists credentials under that account's scoped keys, and appends the account to `UserSettings.accounts`. GitHub credentials retain the legacy compatibility mirror. GitLab PATs are stored only under the `gitlab:<host>#<username>:pat` account key and are never mirrored into the fixed GitHub PAT key.
- GitLab API requests use an HTTPS-only, redirect-blocking transport. Provider switching never copies a GitLab PAT into a GitHub client or legacy GitHub key.
